    ...
    req.onreadystatechange = 
		...
		let rep = JSON.parse(req.responseText);
		    ...
		    for (let obj in rep["contenu"])
                        if (document.getElementById(obj) != null) document.getElementById(obj).innerHTML = rep["contenu"][obj];
                if ("alasuite" in rep)
		    ...
		    for (let obj in rep["classes"])
                        if (document.getElementById(obj) != null) document.getElementById(obj).classList = rep["classes"][obj];
                if ("valeurs" in rep)
		    for (let obj in rep["valeurs"])
                        if (document.getElementById(obj) != null) document.getElementById(obj).value = rep["valeurs"][obj];
		...
		if ("capture_clic" in rep)
			...
			if (taf[0]) document.getElementById(taf[1]).onclick = () => commande_api(taf[2], "objet="+taf[1]);
			else document.getElementById(taf[1]).onclick = undefined;
            }
        };
    req.send();
}
...
#            print('succès')
            ...
            if elts[0] == b'GET': 
                ...
                if comm == chemin_js:
                    ...
                    _actions={}
                    paquet = empaqueter(contenu, "json")
                elif comm == "/__init":  #placé après le catalogue pour pouvoir court-circuiter
                    contenu = bytes(json.dumps(_actions),'utf-8')
                    _actions={}
                    paquet = empaqueter(contenu, "json")
